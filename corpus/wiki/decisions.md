...
## UI interactions & features (locked 2026-06-28 — full grilling pass; detail split across [brief 13 public UI](../briefs/todo/13-ui-interactions-and-features.md), [brief 15 draw-to-filter](../briefs/todo/15-draw-to-filter.md), [brief 16 admin shell/review](../briefs/todo/16-admin-shell-and-review-ui.md))
...
- **/admin (NEW — own contextual menu; [brief 16](../briefs/todo/16-admin-shell-and-review-ui.md)):** separate layout + **sidebar nav** (Sources, Review queue, Places, later Users) + exit-to-public; route-lazy-loaded, admin-gated. **Review screen** = dense filterable table (source→status) + sticky bulk accept/reject toolbar + row→detail drawer with map preview + ambiguous-match resolver.
